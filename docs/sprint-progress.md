...
- **REPRIORITIZED (owner): the EVAL HARNESS is the spine.** Gap: no stored, k-run, before/after eval catching BLAST RADIUS.
- **DONE — Day 13 eval harness** (`a94b487` → `a9b2cd2` → `b29ce73`): `src/kairos/eval/` corpus (507 entries: 161 tau + 20 spotcheck + 15 answers + 311 live; raw-span snapshot so each ref normalizes the SAME fixed input via ITS OWN code; corpus_hash `72fb137f`) + full metric panel + before/after×k=2 + **three-tier gate** (GATE=grounded quality: outcome precision/recall, tau κ; REVIEW=detector precision/recall surfaced not gating; INFO=fire-volume diagnostic) + `eval_runs` append-only store. **Two Fable catches:** (1) panel was VACUOUS — only ran detectors on tau traces, skipped live/owner traces → fixed (raw-span fetch + ref-normalize); (2) gate conflated volume with quality → fixed (three-tier). Retro-validated LIVE: args-fix `3d6a702→9975ecf` PASS (struggle_ratio fires 78→24 = F10 false-positive suppression, INFO; unrecovered_error precision 0.538→0.583 improved; outcome/tau flat); Bug-1 `aead64a→4c30a62` PASS. Already surfaced a REVIEW signal: `coordination_waste.recall 1.0→0.5` (owner's eye later). Gap CLOSED: no change ships without stored k-run before/after + blast-radius.
- **NEXT — Day 14 = haywire flywheel dogfooding the harness** (discovery→owner label the 41 restart traces→detector→ship THROUGH harness→case-study). Agent-sandbox eval = documented next extension. UI deferred (backend first). Plan: `~/.claude/plans/sunny-scribbling-brooks.md`.
