...
- **NEXT — Day 10** Postgres persist (`persist.py`: findings + nightly_rollup, idempotent upserts, redaction, config_hash discipline) + 7-day backfill. **Day 11** delta + minimal dashboard. **Day 12** discovery mode + nightly runner. **Days 13–14** flywheel live → Kairos's own detection-quality delta → `docs/case-study-1.md`.
